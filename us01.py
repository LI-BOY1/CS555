...
def us01():
    ...
    for person in Project.personList:
        ...
        if person.birthDate is not None and birth_date > datetime.now().date():
            print(f"{person.name} was born after current date. BirthDate: {birth_date}, Current date: {current_time}")
            verify = False
        if person.death != 'N/A':
            death = datetime.strptime(datetime.strptime(person.death, '%d %b %Y').strftime("%Y-%m-%d"), '%Y-%m-%d').date()
            if death > current_time:
                print(f"{person.name} was died after current date. Death: {person.death}, Current date: {current_time}")
                verify = False
    for family_member in Project.familyList:
        if family_member.married != 'N/A':
            ...
            if marry_date > current_time:
                print(f"{family_member.husbandName} and {family_member.wifeName} married after current date. Married: {marry_date}, Current Date: {current_time}")
        if family_member.divorce != 'NA':
            ...
            if divorce_date > current_time:
                print(f"{family_member.husbandName} and {family_member.wifeName} divorced after current date. Married: {divorce_date}, Current Date: {current_time}")
    if verify:
